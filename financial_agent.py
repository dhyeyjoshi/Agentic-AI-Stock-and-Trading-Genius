...
st.markdown("<h1 style='text-align: center; font-weight: bold;'>💵 QUANT - Quick Unified Analytics for Next-gen Trading</h1>", unsafe_allow_html=True)
...
def initialize_agents():
    ...
    if not st.session_state.agents_initialized:
        try:
            ...
            st.error(f"⚠️ Error initializing AI agents: {str(e)}")
            return False
...
def main():
                ...
                if initialize_agents():
                    ...
                    with st.spinner(f"Analyzing {stock_symbol}..."):
                        ...
                        if info and hist is not None:
                            ...
                            # Add refresh button
                            # Add a refresh flag in session state if not present
                            if "refresh_flag" not in st.session_state:
                                st.session_state.refresh_flag = False

                            # Refresh Button
                            if st.button("🔄 Refresh Data"):
                                st.session_state.refresh_flag = True  # Set flag instead of rerunning

                            # Check if refresh is needed
                            if st.session_state.refresh_flag:
                                st.session_state.refresh_flag = False  # Reset the flag
                                st.rerun()  # Now it runs safely only once
            except Exception as e:
    ...
    # Footer
    st.markdown("---")
    st.markdown("### 📢 About QUANT - AI-Powered Stock Market Insights")

    st.markdown("""
    **QUANT** is an advanced **AI-driven stock market analysis platform** designed to empower traders, investors, and financial analysts with **real-time insights, predictive analytics, and intelligent decision-making tools.**

    ## 🚀 Why QUANT?
    QUANT leverages a **Multi-Agent Architecture**, where specialized AI agents work together to:
    ...
    ### 🔹 Multi-Agent System:
    QUANT employs dedicated AI agents, each specializing in a **specific task**:
    - **📈 Financial AI Agent** – Fetches stock prices, historical data, analyst recommendations, and trading patterns.
    - **🌍 Web Search Agent** – Gathers financial news, expert opinions, and global market trends.
    - **🤖 Market Analysis Agent** – Aggregates and synthesizes data to provide a **comprehensive trading strategy**.

    ---

    ## 🔥 Key Features:
    ✅ **Real-Time Market Data** – Stay updated with **live stock prices, trading volume, and trends.**  
    ✅ **AI-Powered Predictions** – Get **intelligent forecasts** based on historical and technical analysis.  
    ✅ **Technical & Fundamental Analysis** – Evaluate stocks using **Moving Averages, RSI, Market Capitalization, etc.**  
    ✅ **News & Sentiment Analysis** – AI-curated **financial news, expert opinions, and market sentiment tracking.**  
    ✅ **Multi-Timeframe Analysis** – Analyze stocks across different time periods for **long-term & short-term opportunities.**  
    ✅ **US & Indian Market Support** – Supports **both US and Indian (NSE/BSE) stocks** for a **global trading perspective.**  
    ✅ **Watchlist Management** – Easily **track and monitor your favorite stocks** in one place.  
    ✅ **Interactive Data Visualizations** – Advanced **candlestick charts, line graphs, and heatmaps** for better decision-making.  

    ---

    ## 💡 A Smart & User-Friendly Trading Companion
    QUANT is built with **simplicity and efficiency** in mind. Whether you're a **beginner looking for market insights** or a **professional trader analyzing deep market trends**, our **intuitive interface and AI-backed analytics** help you **make informed decisions effortlessly.**  

    ### 🎯 **What Makes QUANT Different?**
    🔎 **Customize Your Analysis** – Use the **sidebar** to **personalize your analysis** and **manage your watchlist.**  
    ⚡ **Fast & Efficient** – **AI agents work behind the scenes** to fetch, process, and visualize financial data instantly.  
    📊 **Seamless Data Integration** – Combines **real-time stock prices, news, and predictive analytics** in one **dashboard.**  

    ---
    **🔹 Stay Ahead in the Market with QUANT!** 🚀💹
    """, unsafe_allow_html=True)
